...
public class GameController {
    ...
    public void mouseClickCell(int row, int col) {
        if (this.currentBuilding != null && map.canPlaceBuilding(this.currentBuilding, row, col)) {
            ...
            Image[][] splitedImage = ImageSplitter.splitImages(new Image(this.currentBuilding.getImagePath()),
                    this.currentBuilding.getSize(), this.currentBuilding.getSize());
            ...
            for (int r = 0; r < this.currentBuilding.getSize(); r++) {
                for (int c = 0; c < this.currentBuilding.getSize(); c++) {
                    ...
                    if (targetRow >= 0 && targetRow < map.getHEIGHT() && targetCol >= 0 && targetCol < map.getWIDTH()) {
                        // Rectangle tile = mainView.getGameArea().getGameGrid().getTile(targetRow,
                        // targetCol);
                        mainView.getGameArea().getGameGrid().setTileBackgroundImage(targetRow, targetCol,
                                splitedImage[r][c]);
                    }
                }
            }
